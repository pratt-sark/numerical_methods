...
void gauJacobi(int **a, int n, double *x);
...
void main()
{
    ...
    printf("\n\n-----SOLUTION-------\n");
    gauJacobi(a,n,result);
}

//returns column index of maximum value in row 'i'
...
//returns true if row 'row' is diagonally dominant
bool diag_dom(int **a, int n, int row)
{
    int j,sum=0;
    for(j=0;j<n;j++)
    {
        if (row!=j) //sum of all elements except the pivot element
            sum+=abs(a[row][j]);
    }
...
bool check_diag(int **a, int n)
{
    ...
    else
    {
        ...
        for(i=0;i<n;i++)
        {
            ...
            {
                //printf("\nin if\n");
                //get column index of maximum value in row 'i'
...
//to find solution vector using Gauss-Jacobi Method
void gauJacobi(int **a, int n, double *x)
{
    int i,j,k=1;
    ...
    {
        //for debugging
        //printf("\nFor Iteration %d, previous values used: \n",k);
        //display1d(x_in,n);
        for(i=0;i<n;i++)
        {
            for(j=0;j<n;j++)
            {
                if (j!=i) //excluding the diagonal element
                    x[i] = x[i] + (-1.0*a[i][j]*x_in[j]); //formula for Gauss-Jacobi
            }
    ...
    display1d(x,n); //display the solution vector
}

//to allocate memory of a matrix of dimensions x,x+1
...
//to accept elements of a matrix
...
//display an array of real numbers
void display1d(double *a, int n)
{
    int i;
    for(i=0;i<n;i++)
